...
public class MessageDTO extends BaseDTO {
	...
	//ForgotePassword loginId,password;->backend se message
	...
	public String getBody(HashMap<String, String> params) {
		...
		/* this.body___vale---->backend se Message;
		 * //<HTML><BODY> "   <H1>Your password is reccovered !! </H1>"
		 * We have recovered your password and here it is <br> <h3> User - {user}
		 * Password - {password} </h3> </BODY></HTML>
		 */
		...
		while (it.hasNext()) {
			...
			System.out.println("");
			System.out.println("2. Replace Key  of  Value:-");
			System.out.println(" [ params.get(key)]  =="+params.get(key));
			System.out.println("");
			System.out.println("3. replace the value in the text:-");
			System.out.println("      text.replace({"  + key +  "}, params.get(key) = "+params.get(key));
