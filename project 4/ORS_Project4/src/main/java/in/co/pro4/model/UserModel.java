...
import in.co.pro4.exception.UserNotFound;
...
public class UserModel {
	...
	public static void delete(UserBean bean) throws ApplicationException {
		...
		try {
			conn = JDBCDataSource.getConnection();
			conn.setAutoCommit(false);
			PreparedStatement pstmt = conn.prepareStatement("DELETE FROM ST_USER WHERE firstName=?");
	 ...
	 *
	 */
	public UserBean authenticate(String login, String password) throws ApplicationException ,UserNotFound{
		...
		try {
			...
			if(bean==null) {
				throw new UserNotFound("Invalid Login And Password..!!!!");
			}
