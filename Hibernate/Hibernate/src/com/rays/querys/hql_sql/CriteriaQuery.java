...
// 9-{CRITERIA}-Join
		 ...
		 * 
		 * crit.setFetchMode("ITEME_ID", FetchMode.JOIN); crit.add(Restrictions.eq("id",
		 * 2L));
		 * 
		 * List list = crit.list(); Iterator it = list.iterator();
		 * 
		 * Auction Aucti = (Auction) it.next(); System.out.println(Aucti.getId() + " " +
		 * Aucti.getDescription()); ;
		 * 
		 * Set s = Aucti.getBids(); Iterator jointdata = s.iterator();
		 * 
		 * while (jointdata.hasNext()) {
		 * 
		 * Bid bid = (Bid) jointdata.next();
		 * 
		 * System.out.println(bid.getId() + "  " + bid.getAmount() + "  " +
		 * bid.getTimeStamp());
		 * 
		 * }
		 */
